...
| 문제점 | Chalk의 해결책 |
|--------|---------------|
| 튜터가 자신의 역량을 증명하기 어려움 | 실제 수업 데이터 기반의 "위조 불가능한" 포트폴리오 자동 생성 |
...
**장점**: 핵심 가치인 "Unfakeable Portfolio"를 더 강화

| 기능 | 설명 |
|------|------|
| **과목 간 상관관계 분석** | "수학 성적이 좋은 학생이 물리도 잘한다" 같은 인사이트 |
| **학습 스타일 분류** | AI가 수업 패턴을 분석하여 Visual/Auditory/Kinesthetic 분류 |
| **동료 비교 벤치마크** | 익명화된 데이터로 "상위 20%" 같은 상대 위치 표시 |
| **예측 정확도 피드백 루프** | AI 예측과 실제 결과를 비교하여 모델 개선 |
| **장기 성장 곡선** | 6개월, 1년 단위의 성장 트렌드 시각화 |
